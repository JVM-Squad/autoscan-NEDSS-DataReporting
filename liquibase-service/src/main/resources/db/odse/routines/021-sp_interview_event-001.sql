...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT 
            ...
            cvg3.code_short_desc_txt AS IX_TYPE,
        ...
        FROM NBS_ODSE.dbo.INTERVIEW ix
            ...
            LEFT JOIN nbs_srte.dbo.Code_value_general cvg4
                ON ix.interview_loc_cd = cvg4.code and cvg4.code_set_nm in ('NBS_INTVW_LOC','NBS_INTVW_LOC_STDHIV')
        ...
        VALUES (@batch_id, 'Interview PRE-Processing Event', 'nrt_interview', 'START', @Proc_Step_no, @Proc_Step_Name, @RowCount_no);
      ...
      WHERE 
        ...
        WHERE 
            nbs_answer_uid NOT IN (SELECT nbs_answer_uid FROM #coded_table_snm);
...
WHERE RDB_TABLE_NM = 'D_INTERVIEW' 
    ...
    AND ANSWER_GROUP_SEQ_NBR IS NULL
        ...
        SET
         ...
         SELECT
    COALESCE(SNM.CODE_SET_GROUP_ID, NONSNM.CODE_SET_GROUP_ID) AS CODE_SET_GROUP_ID,
    ...
    SNT.ANSWER_VALUE,
    CASE WHEN TRIM(NONSNM.ANSWER_TXT1) = '' THEN COALESCE(SNT.ANSWER_TXT, SNM.ANSWER_TXT, NONSNM.ANSWER_TXT)
...
AND aa.NBS_QUESTION_UID = ctsm.NBS_QUESTION_UID;
        ...
        INTO 
            #CODED_COUNTY_TABLE
        FROM 
            #CODED_TABLE_SN_MERGED AS CODED
        LEFT JOIN 
        ...
        LEFT JOIN 
            ...
            AND CVG.CODE = CODED.ANSWER_TXT
        WHERE 
            METADATA.CODE_SET_NM = 'COUNTY_CCD';
...
BEGIN
        ...
        SET
            @PROC_STEP_NAME = 'GENERATING #CODED_TABLE_FINAL';
                ...
                SELECT
                COALESCE(CTO.RDB_COLUMN_NM, CCT.RDB_COLUMN_NM, CTSM.RDB_COLUMN_NM) AS RDB_COLUMN_NM,
        ...
        ANSWER_TXT,   ACT_UID AS INTERVIEW_UID ,
        RECORD_STATUS_CD, NBS_QUESTION_UID,
        data_type,
        rdb_table_nm,
        answer_group_seq_nbr
        from
            dbo.v_rdb_ui_metadata_answers
            WHERE ACT_UID IN (SELECT value FROM STRING_SPLIT(@ix_uids, ','))
        ) as metadata
...
FROM 
   dbo.v_rdb_ui_metadata_answers
WHERE 
    RDB_TABLE_NM = 'D_INTERVIEW'
    AND QUESTION_GROUP_SEQ_NBR IS NULL
    AND UPPER(DATA_TYPE) = 'TEXT'
    AND data_location = 'NBS_ANSWER.ANSWER_TXT'
    AND ACT_UID IN (SELECT value FROM STRING_SPLIT(@ix_uids, ','))
) metadata
LEFT JOIN 
...
FROM 
    #NUMERIC_BASE_DATA;
        ...
        NBS_ANSWER_UID,
    CODE_SET_GROUP_ID,
    CASE WHEN LEN(RDB_COLUMN_NM2) > 0 THEN RDB_COLUMN_NM2
    ELSE RDB_COLUMN_NM
    END AS RDB_COLUMN_NM,
    ANSWER_TXT,
    INTERVIEW_UID,  
...
FROM 
    ...
    BEGIN
        ...
        SELECT 
            COALESCE(B.NBS_ANSWER_UID, A.NBS_ANSWER_UID) AS NBS_ANSWER_UID,
...
WHERE 
    ...
    AND PA.ANSWER_GROUP_SEQ_NBR IS NULL;
        ...
        SET
            @PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
            @PROC_STEP_NAME = 'GENERATING #PIVOTED_DATA';

        SELECT INTERVIEW_UID,
        IX_900_SITE_ID, IX_900_SITE_ZIP,IX_CONTACTS_NAMED_IND, IX_900_SITE_TYPE, CLN_CARE_STATUS_IXS, IX_INTERVENTION
        INTO #PIVOTED_DATA
        FROM (
            SELECT 
                INTERVIEW_UID,
        ...
        PIVOT (
            MAX(ANSWER_VAL) 
            FOR RDB_COLUMN_NM IN ([IX_900_SITE_ID], [IX_900_SITE_ZIP], [IX_CONTACTS_NAMED_IND], [IX_900_SITE_TYPE], [CLN_CARE_STATUS_IXS], [IX_INTERVENTION])
        ) AS PivotTable
        ...
        SELECT DISTINCT 
            ...
            TRY_CAST(
                SUBSTRING(
                    ...
                    CHARINDEX('~', ANSWER_TXT + '~', CHARINDEX('~', ANSWER_TXT) + 1) - CHARINDEX('~', ANSWER_TXT) - 1
                ) AS datetime
            ) AS COMMENT_DATE
        INTO #INTERVIEW_NOTE
        FROM #INTERVIEW_NOTE_INIT
                          ...
                          SELECT
                              (
                                  ...
                                  FROM
                                      #UNIONED_DATA ud 
                                  WHERE
                                      ud.interview_uid = ix.interview_uid
                          ...
                          SELECT
                              (
                                  SELECT
                                    ...
                                    COMMENT_DATE
                                  FROM
                                      #INTERVIEW_NOTE ixnote 
                                  WHERE
                                      ixnote.interview_uid = ix.interview_uid
                                       FOR json path,INCLUDE_NULL_VALUES
                              ) AS notes
                      ) AS notes
           ) AS nesteddata
