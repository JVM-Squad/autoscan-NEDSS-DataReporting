...
@Proc_Step_no = @Proc_Step_no + 1;
        SET
@Proc_Step_Name = 'Creating LAB_INV_MAP';
...
FROM dbo.INVESTIGATION with (nolock)
                     LEFT OUTER JOIN dbo.CASE_COUNT
with (nolock)
...
FROM dbo.INVESTIGATION inv with (nolock)
                     LEFT OUTER JOIN dbo.CASE_COUNT cc
with (nolock)
ON
...
-- Create session table for patient info
        IF
OBJECT_ID('tempdb..#TMP_CLDM_GEN_PATIENT_ADD') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_GEN_PATIENT_ADD';
...
from #TMP_CLDM_ALL_CASE as GC with (nolock)
                 left join dbo.D_PATIENT as p
with (nolock)
ON GC.PATIENT_KEY = p.PATIENT_key
    left join dbo.CONDITION as C
with (nolock)
...
if
@debug = 'true'
select '#TMP_CLDM_GEN_PATIENT_ADD', *
from #TMP_CLDM_GEN_PATIENT_ADD;
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_GEN_PAT_ADD_INV') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_GEN_PAT_ADD_INV';

        /*Notes: Event metric is not part of RTR yet.*/
select GPA.*,
       i.INV_LOCAL_ID 'INV_LOCAL_ID', i.INVESTIGATION_STATUS 'INVESTIGATION_STATUS', i.INV_CASE_STATUS 'INV_CASE_STATUS', i.JURISDICTION_NM AS INV_JURISDICTION_NM,
       i.ILLNESS_ONSET_DT 'ILLNESS_ONSET_DT', i.INV_START_DT 'INV_START_DT', i.INV_RPT_DT 'INV_RPT_DT', i.RPT_SRC_CD_DESC 'RPT_SRC_CD_DESC', i.EARLIEST_RPT_TO_CNTY_DT 'EARLIEST_RPT_TO_CNTY_DT', i.EARLIEST_RPT_TO_STATE_DT 'EARLIEST_RPT_TO_STATE_DT', i.DIE_FRM_THIS_ILLNESS_IND 'DIE_FRM_THIS_ILLNESS_IND', i.outbreak_ind 'OUTBREAK_IND', i.DISEASE_IMPORTED_IND,
       ...
       i.CASE_RPT_MMWR_WK,
       i.CASE_RPT_MMWR_YR 'CASE_RPT_MMWR_YR', i.DIAGNOSIS_DT 'DIAGNOSIS_DT', i.HSPTLIZD_IND 'HSPTLIZD_IND', i.HSPTL_ADMISSION_DT 'HSPTL_ADMISSION_DT', i.HSPTL_DISCHARGE_DT,
       i.HSPTL_DURATION_DAYS,
       i.Transmission_mode,
       i.CASE_OID,
       i.INV_COMMENTS 'INV_COMMENTS', em.ADD_TIME AS INV_ADD_TIME,
...
from #TMP_CLDM_GEN_PATIENT_ADD as GPA with (nolock)
                 left join dbo.investigation as i
with (nolock)
...
if
@debug = 'true'
select '#TMP_CLDM_GEN_PAT_ADD_INV', *
from #TMP_CLDM_GEN_PAT_ADD_INV;
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_GEN_PATCOMPL_INV_PROVIDER', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_GEN_PATCOMPL_INV_PROVIDER';
...
FROM #TMP_CLDM_GEN_PAT_ADD_INV AS GPI with (nolock)
                 LEFT JOIN dbo.D_PROVIDER AS PP
with (nolock)
ON GPI.PHYSICIAN_KEY = PP.PROVIDER_KEY;

if
@debug = 'true'
select '#TMP_CLDM_GEN_PATCOMPL_INV_PROVIDER', *
from #TMP_CLDM_GEN_PATCOMPL_INV_PROVIDER;
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_GEN_PATCOMPL_INV_INVESTIGATOR', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_GEN_PATCOMPL_INV_INVESTIGATOR';
...
FROM #TMP_CLDM_GEN_PATCOMPL_INV_PROVIDER AS GPI with (nolock)
                 LEFT JOIN dbo.D_ORGANIZATION AS O
with (nolock)
ON GPI.RPT_SRC_ORG_KEY = O.ORGANIZATION_KEY;

if
@debug = 'true'
select '#TMP_CLDM_GEN_PATCOMPL_INV_INVESTIGATOR', *
from #TMP_CLDM_GEN_PATCOMPL_INV_INVESTIGATOR;
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_GEN_PATINFO_INV_PHY_RPTSRC_COND', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_GEN_PATINFO_INV_PHY_RPTSRC_COND';
...
if
@debug = 'true'
select '#TMP_CLDM_GEN_PATINFO_INV_PHY_RPTSRC_COND', *
...
set CONFIRMATION_DT = (SELECT min(CMG.[CONFIRMATION_DT])
                       FROM dbo.[CONFIRMATION_METHOD_GROUP] CMG
with (nolock)
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'UPDATE EVENT DATES';
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_CASE_LAB_DATAMART', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_CASE_LAB_DATAMART';
...
if
@debug = 'true'
select '#TMP_CLDM_CASE_LAB_DATAMART', *
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_invlab', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_invlab';
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_lab', 'U') IS NOT NULL
DROP TABLE #TMP_CLDM_lab;

IF
OBJECT_ID('tempdb..#TMP_CLDM_both', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_both';
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_inv2labs', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_inv2labs';
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_invmorb', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_invmorb';
...
ON ME.INVESTIGATION_KEY = I.INVESTIGATION_KEY
    ...
    INNER JOIN dbo.MORBIDITY_REPORT MR
with (nolock)
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_morbResults', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_morbResults';
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_morbLabResults', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_morbLabResults';
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_Inv2labs_final', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_Inv2labs_final';
...
-- Sample1
IF
OBJECT_ID('tempdb..#TMP_CLDM_sample1', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_sample1';
...
-- Sample2
IF
OBJECT_ID('tempdb..#TMP_CLDM_sample2', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_sample2';

select investigation_key as 'KEY', lab_test_key as 'SUBKEY', FORMAT(LAB_RPT_RECEIVED_BY_PH_DT, 'MM/dd/yyyy ') as C1,
       FORMAT(SPECIMEN_COLLECTION_DT, 'MM/dd/yyyy ') as C2,
       rtrim(RESULTED_LAB_TEST_CD_DESC) as 'C3', rtrim(RESULTEDTEST_VAL_CD_DESC) as 'C4', rtrim(NUMERIC_RESULT_WITHUNITS) as 'C5', substring(rtrim(LAB_RESULT_TXT_VAL), 1, 200) as 'C6', substring(rtrim(LAB_RESULT_COMMENTS), 1, 200) as 'C7', rtrim(LAB_RPT_LOCAL_ID) as 'C8', rtrim(ELR_IND) as 'c9'
into #TMP_CLDM_sample2
from #TMP_CLDM_inv2labs_final;

CREATE
NONCLUSTERED INDEX [idx_tmp_sample2_key]
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_sample21', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_sample21';
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_sample3', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_sample3';
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_sample4', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_sample4';

SELECT [key], bigChunk = STUFF((SELECT DISTINCT ' <br><br>' + bigChunk
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_sample5', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_sample5';
...
IF
OBJECT_ID('tempdb..#TMP_SPECIMEN_COLLECTION_TABLE', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_SPECIMEN_COLLECTION_TABLE';

select investigation_key as 'KEY', min(SPECIMEN_COLLECTION_DT) as SPECIMEN_COLLECTION_DT
...
IF
OBJECT_ID('tempdb..#TMP_CLDM_CASE_LAB_DATAMART_FINAL', 'U') IS NOT NULL
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CLDM_CASE_LAB_DATAMART_FINAL';
...
ON tcld.investigation_key = ts5.investigation_key
    LEFT OUTER JOIN #TMP_SPECIMEN_COLLECTION_TABLE tspt
with (nolock)
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING CASE_LAB_DATAMART';

        -- Create or Update final CASE_LAB_DATAMART table
        IF
OBJECT_ID('dbo.CASE_LAB_DATAMART', 'U') IS NULL
BEGIN
...
(
    ...
    [PATIENT_FIRST_NM] [varchar]
(
    50
) NULL,
    [PATIENT_MIDDLE_NM] [varchar]
(
    50
) NULL,
    [PATIENT_LAST_NM] [varchar]
(
    50
) NULL,
...
(
    ...
    [LABORATORY_INFORMATION] [varchar]
(
    4000
) NULL,
...
where investigation_key in (SELECT li.[INVESTIGATION_KEY]
                            FROM dbo.[S_INVESTIGATION] si
                                             JOIN dbo.[L_INVESTIGATION] li
ON si.CASE_UID = li.CASE_UID
where RECORD_STATUS_CD = 'INACTIVE');

-- Insert new/updated records
insert into [dbo].CASE_LAB_DATAMART

SELECT distinct [INVESTIGATION_KEY], [PATIENT_LOCAL_ID], [INVESTIGATION_LOCAL_ID], [PATIENT_FIRST_NM], [PATIENT_MIDDLE_NM], [PATIENT_LAST_NM], [PATIENT_STREET_ADDRESS_1], [PATIENT_STREET_ADDRESS_2], [PATIENT_CITY], [PATIENT_STATE], [PATIENT_ZIP], [PATIENT_COUNTY], [PATIENT_HOME_PHONE], [PATIENT_DOB], [AGE_REPORTED], [AGE_REPORTED_UNIT], [PATIENT_CURRENT_SEX], [RACE], [JURISDICTION_NAME], [PROGRAM_AREA_DESCRIPTION], [INVESTIGATION_START_DATE], [CASE_STATUS], [DISEASE], [DISEASE_CD], [REPORTING_SOURCE], [GENERAL_COMMENTS], [PHYSICIAN_NAME], [PHYSICIAN_PHONE], cast ([LABORATORY_INFORMATION] as varchar (4000)), [PROGRAM_JURISDICTION_OID], [PHC_ADD_TIME], [PHC_LAST_CHG_TIME], [EVENT_DATE], EARLIEST_SPECIMEN_COLLECTION_DT, EVENT_DATE_TYPE
...
@PROC_STEP_NO = @PROC_STEP_NO + 1;
        SET
@PROC_STEP_NAME = 'GENERATING TMP_CASE_LAB_DATAMART_MODIFIED';

        IF
OBJECT_ID('tempdb..#TMP_CASE_LAB_DATAMART_MODIFIED', 'U') IS NOT NULL
...
TRANSACTION;
        ...
        IF
OBJECT_ID('tempdb..#TEMP_UPDATED_LAB_INV_MAP', 'U') IS NOT NULL
DROP TABLE #TEMP_UPDATED_LAB_INV_MAP;
IF
OBJECT_ID('tempdb..#TMP_CLDM_All_Case', 'U') IS NOT NULL
DROP TABLE #TMP_CLDM_All_Case;
IF
OBJECT_ID('tempdb..#TMP_CLDM_GEN_PATIENT_ADD', 'U') IS NOT NULL
DROP TABLE #TMP_CLDM_GEN_PATIENT_ADD;
IF
OBJECT_ID('tempdb..#TMP_CLDM_GEN_PAT_ADD_INV', 'U') IS NOT NULL
DROP TABLE #TMP_CLDM_GEN_PAT_ADD_INV;
IF
OBJECT_ID('tempdb..#TMP_CLDM_GEN_PATCOMPL_INV_PROVIDER', 'U') IS NOT NULL
DROP TABLE #TMP_CLDM_GEN_PATCOMPL_INV_PROVIDER;
...
INSERT INTO [dbo].[job_flow_log] (batch_id,
    ...
    [step_name],
[row_count])
VALUES (@batch_id, 'CASE_LAB_DATAMART', 'CASE_LAB_DATAMART', 'COMPLETE', @Proc_Step_no, @Proc_Step_name, @RowCount_no);
...
INSERT INTO [dbo].[job_flow_log] (batch_id,
    ...
    [Error_Description],
[row_count])
VALUES (@batch_id, 'CASE_LAB_DATAMART', 'CASE_LAB_DATAMART', 'ERROR', @Proc_Step_no, 'ERROR - ' + @Proc_Step_Name, 'Step -' + CAST (@Proc_Step_no AS VARCHAR (3)) + ' -' + CAST (ERROR_MESSAGE() AS VARCHAR (500)), 0);
