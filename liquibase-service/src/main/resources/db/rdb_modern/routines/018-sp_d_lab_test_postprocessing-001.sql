CREATE
OR
ALTER PROCEDURE dbo.sp_d_lab_test_postprocessing
...
@Proc_Step_Name VARCHAR(200) = '' ;
    DECLARE
@batch_start_time datetime2(7) = null ;
    DECLARE
@batch_end_time datetime2(7) = null ;
    DECLARE
@batch_id BIGINT;
    SET
...
set jurisdiction_nm = (select code_short_desc_txt
                       from nbs_srte..jurisdiction_code
                       where code = #LAB_TESTinit_a.Jurisdiction_cd
                         and code_set_nm = 'S_JURDIC_C')
...
set document_link =('<a href="#" ' +
                    replace(('onClick="window.open(''/nbs/viewELRDocument.do?method=viewELRDocument&documentUid='
...
update #LAB_TEST1
set #LAB_TEST1.PROCESSING_DECISION_DESC = cvg.[code_short_desc_txt] from nbs_srte..Code_value_general cvg,
									  #LAB_TEST1 tlt1
...
set jurisdiction_nm = (select code_short_desc_txt
                       from nbs_srte..jurisdiction_code
                       where code = #Merge_Order.Jurisdiction_cd
                         and code_set_nm = 'S_JURDIC_C')
where Jurisdiction_cd is not null
;
...
(
    1,
    1
) NOT NULL, [LAB_TEST_UID] [numeric](20, 0) NULL, [LAB_TEST_KEY] [numeric](18, 0) NULL
											 ) ON [PRIMARY]
;
...
UPDATE #D_LAB_TEST_N
SET JURISdiction_nm = NULL
where JURISdiction_nm = '';

UPDATE #D_LAB_TEST_N
SET [ACCESSION_NBR_merge] = NULL
where [ACCESSION_NBR_merge] = '';

UPDATE #D_LAB_TEST_N
SET [SPECIMEN_DESC] = NULL
where [SPECIMEN_DESC] = '';

UPDATE #D_LAB_TEST_N
SET [SPECIMEN_SRC] = NULL
where [SPECIMEN_SRC] = '';

UPDATE #D_LAB_TEST_N
SET [CLINICAL_INFORMATION] = NULL
where [CLINICAL_INFORMATION] = '';

UPDATE #D_LAB_TEST_N
SET REASON_FOR_TEST_DESC = NULL
where REASON_FOR_TEST_DESC = '';

UPDATE #D_LAB_TEST_N
SET REASON_FOR_TEST_CD = NULL
where REASON_FOR_TEST_CD = '';
...
SET lt.[LAB_TEST_STATUS]                = rtrim(cast(dltu.LAB_TEST_STATUS AS varchar(50)))
  ...
  , lt.[PROCESSING_DECISION_DESC]       = rtrim(cast(dltu.PROCESSING_DECISION_DESC AS varchar(50))) FROM dbo.LAB_TEST lt, #D_LAB_TEST_U dltu
...
(
    1,
    1
) NOT NULL, [LAB_TEST_Key] [bigint]  NULL, [LAB_TEST_uid] [bigint] NULL, [COMMENTS_FOR_ELR_DT] [datetime] NULL, [USER_COMMENT_CREATED_BY] [bigint] NULL, [USER_RPT_COMMENTS] [varchar](8000) NULL, [RECORD_STATUS_CD] [varchar](8) NOT NULL, [observation_uid] [bigint] NOT NULL, USER_COMMENT_KEY [bigint], [RDB_LAST_REFRESH_TIME] [datetime] NULL
								) ON [PRIMARY]
;
...
select distinct tdltn.LAB_TEST_Key,
                ...
                lab214.activity_to_time 'COMMENTS_FOR_ELR_DT' , lab214.add_user_id 'USER_COMMENT_CREATED_BY' , REPLACE(REPLACE(ovt.ovt_value_txt, CHAR(13), ' '), CHAR(10), ' ') 'USER_RPT_COMMENTS',--TRANSLATE(ovt.value_txt,' ' ,'0D0A'x)	'USER_RPT_COMMENTS' 
...
UPDATE #Lab_Rpt_User_Comment_N
SET USER_COMMENT_KEY= [LAB_COMMENT_id] + coalesce ((SELECT MAX (USER_COMMENT_KEY) FROM dbo.Lab_Rpt_User_Comment), 1)
...
(
    1,
    1
) NOT NULL, [LAB_TEST_uid] [bigint] NULL, [COMMENTS_FOR_ELR_DT] [datetime] NULL, [USER_COMMENT_CREATED_BY] [bigint] NULL, [USER_RPT_COMMENTS] [varchar](8000) NULL, [RECORD_STATUS_CD] [varchar](8) NOT NULL, [observation_uid] [bigint] NOT NULL, [RDB_LAST_REFRESH_TIME] [datetime] NULL
								) ON [PRIMARY]
;
...
select distinct tdltn.lab_rpt_uid as LAB_TEST_uid,
                lab214.activity_to_time 'COMMENTS_FOR_ELR_DT' , lab214.add_user_id 'USER_COMMENT_CREATED_BY' , REPLACE(REPLACE(ovt.ovt_value_txt, CHAR(13), ' '), CHAR(10), ' ') 'USER_RPT_COMMENTS',--TRANSLATE(ovt.value_txt,' ' ,'0D0A'x)	'USER_RPT_COMMENTS' , tdltn.record_status_cd 'RECORD_STATUS_CD' , lab214.observation_uid,
                getdate()
...
UPDATE
    lruc
SET lruc.[USER_RPT_COMMENTS]       = lrucu.USER_RPT_COMMENTS
  ...
  , lruc.[RDB_LAST_REFRESH_TIME]   = lrucu.RDB_LAST_REFRESH_TIME FROM dbo.Lab_Rpt_User_Comment lruc, #LAB_RPT_USER_COMMENT_U lrucu
