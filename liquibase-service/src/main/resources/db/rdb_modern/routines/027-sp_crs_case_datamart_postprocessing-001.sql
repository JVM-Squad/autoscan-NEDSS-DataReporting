...
BEGIN
    ...
    BEGIN TRY
        ...
        where 
            inv.public_health_case_uid in (SELECT value FROM STRING_SPLIT(@inv_uids, ','))
...
FROM (SELECT DISTINCT col_nm FROM #OBS_NUMERIC) AS cols;
        ...
        SELECT @obstxt_columns = COALESCE(STRING_AGG(CAST(QUOTENAME(col_nm) AS NVARCHAR(MAX)), ',') WITHIN GROUP (ORDER BY col_nm) , '')
...
FROM (SELECT DISTINCT col_nm FROM #OBS_NUMERIC) AS cols;
        ...
        SELECT @obstxt_insert_columns = COALESCE(STRING_AGG('ovt.' + CAST(QUOTENAME(col_nm) AS NVARCHAR(MAX)), ',') WITHIN GROUP (ORDER BY col_nm) , '')
FROM (SELECT DISTINCT col_nm FROM #OBS_TXT) AS cols;
