CREATE OR ALTER PROCEDURE dbo.sp_d_lab_test_postprocessing_copy @obs_ids nvarchar(max),
                                                           @debug bit = 'false'
as
...
BEGIN
    ...
    BEGIN TRY
        ...
        ALTER TABLE #Morb_OID
            DROP COLUMN Lab_Rpt_Uid_Test1,LAB_TEST_uid,LAB_TEST_uid_oth;
        ...
        -- can just be done in the select statement via case
        update #LAB_TEST1
        set oid = morb_oid
        where rtrim(morb_oid) is not null;

        -- probably just left join?
        update #LAB_TEST1
...
-- create table Result_to_Order as
        ...
        select distinct dimc.LAB_TEST_uid                                                 as LAB_TEST_uid_final_order,
               ...
               tlt1.lab_rpt_uid,
                ...
                tlt1.Danger_cd,
                tlt1.Danger_cd_desc,
                CASE WHEN LAB_TEST_TYPE = 'Order' THEN LAB_TEST_pntr
        ...
        select tst.LAB_TEST_uid_final_order,
                ...
                tst.lab_rpt_share_ind,
                tst.oid,
                case when tst.record_status_cd = '' then tst.record_status_cd_for_result_drug
...
/*note: When the OID is null that means this lab report is needing assignment of jurisdiction*/
        ...
        select distinct lab.LAB_TEST_uid_final_order,
                        ...
                        lab.LAB_TEST_pntr,
                        CASE WHEN lab.LAB_TEST_TYPE = 'Result' then lab.resulted_lab_report_date
                             WHEN lab.LAB_TEST_TYPE = 'Order_rslt' then lab.sus_lab_report_date
                             ELSE lab.LAB_TEST_dt
                        END AS LAB_TEST_DT,
                        lab.test_method_cd,
                        lab.root_ordered_test_pntr,
                        CASE WHEN rtrim(lab.test_method_cd_desc) = '' THEN NULL
                        ...
                        lab.oid,
                        CASE WHEN lab.record_status_cd IN ('', 'UNPROCESSED', 'UNPROCESSED_PREV_D', 'PROCESSED') or lab.record_status_cd is null then  'ACTIVE'
        ...
        select lt.root_ordered_test_pntr                                                           as root_ordered_test_pntr_merge,
               ...
               obs.ADD_TIME                                                                        as LAB_RPT_CREATED_DT,
               obs.JURISDICTION_CD,
               CASE WHEN obs.jurisdiction_cd IS NOT NULL THEN jc.code_short_desc_txt
               ...
               obs.target_site_desc_txt                                                            as SPECIMEN_SITE_desc,
               obs.local_id                                                                        as lab_rpt_local_id,
               CASE WHEN obs.record_status_cd IN ('', 'UNPROCESSED', 'UNPROCESSED_PREV_D', 'PROCESSED') THEN 'ACTIVE'
                    WHEN obs.record_status_cd = 'INACTIVE' THEN 'LOG_DEL'
               ELSE obs.record_status_cd
                END as record_status_cd_merge,
                CASE WHEN COALESCE(obs2.program_jurisdiction_oid, obs.program_jurisdiction_oid, lt.order_oid) = 4 THEN NULL
                ELSE COALESCE(obs2.program_jurisdiction_oid, obs.program_jurisdiction_oid, lt.order_oid) END as order_oid
        into #Merge_Order
        from #LAB_TEST lt
        ...
        -- all of the below can be case statements
        update #LAB_TEST
        ...
        select  lt1.LAB_TEST_ptnr,
                ...
                lto.LAB_TEST_uid_oth,
                lto.interpretation_flg,
                CASE WHEN lto.REASON_FOR_TEST_DESC = '' THEN NULL
                    ELSE lto.REASON_FOR_TEST_DESC
                END AS REASON_FOR_TEST_DESC,
                CASE WHEN lto.REASON_FOR_TEST_CD = '' THEN NULL
                ...
                lto.test_method_cd_desc,
                lto.priority_cd,
                CASE WHEN lto.CLINICAL_INFORMATION = '' THEN NULL
                ...
                lto.lab_rpt_share_ind,
                lto.oid,
                CASE WHEN ltmi.record_status_cd_merge IS NOT NULL THEN ltmi.record_status_cd_merge
                ...
                case when ltmi.ELR_IND1 IS NOT NULL THEN ltmi.ELR_IND1
                    ELSE lto.ELR_IND
                END AS ELR_IND,
                CASE WHEN ltmi.ACCESSION_NBR_merge = '' THEN NULL
                ...
                CASE WHEN ltmi.JURISDICTION_NM = '' THEN NULL
                ...
                CASE WHEN ltmi.SPECIMEN_SRC = '' THEN NULL
                    ELSE ltmi.SPECIMEN_SRC
                END AS SPECIMEN_SRC,
                ltmi.specimen_site,
                CASE WHEN ltmi.SPECIMEN_DESC = '' THEN NULL
...
---------------------------------------------------------*/
        ...
        select distinct tdltn.LAB_TEST_Key,
                        ...
                        CASE WHEN REPLACE(REPLACE(ovt.ovt_value_txt, CHAR(13), ' '), CHAR(10), ' ') = '' THEN NULL
